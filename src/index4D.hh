...
class Index4D : public std::array<int,4> {
 public:
    ...
    void set(int d, int dp, MType type) {
        lend(type)=d;
        rend(type)=dp;
    }

    //! @brief difference between left and right index corresponding to type
    int difference(MType type) const {
        return rend(type)-lend(type);
    }

    void add(int d, int dp, MType type) {
        lend(type)+=d;
        rend(type)+=dp;
    }

    //!@ shrink fragment at position of type
    void shrink(int d, int dp, MType type) {
        switch(type) {
...
inline Index4D& Index4D::operator +=(const Index4D &x) {
    i()+=x.i();
    j()+=x.j();
    k()+=x.k();
    l()+=x.l();
...
inline std::ostream& operator <<(std::ostream &out, const Index4D &x) {
