...
const char *args_info_help[] = {
  ...
  "  -i, --input-file       Give a path to an input file containing the sequence",
...
static void init_args_info(struct args_info *args_info)
{
  ...
  args_info->noConv_help = args_info_help[6] ;
}
void
cmdline_parser_print_version (void)
{
  ...
  printf ("%s %s\n",
     (strlen(CCJ_CMDLINE_PACKAGE_NAME) ? CCJ_CMDLINE_PACKAGE_NAME : "CCJ"),
     CCJ_CMDLINE_VERSION);
  ...
  while (1)
    {
      ...
      c = getopt_long (argc, argv, "hVvd:P:i:", long_options, &option_index);
